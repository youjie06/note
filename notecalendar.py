...
class CalendarApp:
    ...
    def create_widgets(self):
        # 創建主要布局
        main_frame = tk.Frame(self.master, bg="white")  # 創建一個白色背景的框架
        main_frame.pack(padx=10, pady=10)  # 將框架放置到主視窗中，並設置內邊距

        # 年份和月份布局
        year_month_frame = tk.Frame(main_frame, bg="white")  # 在主框架中創建一個白色背景的子框架
        year_month_frame.grid(row=0, column=0, columnspan=7, pady=(0, 10))  # 將子框架放置到主框架中，設置在第一行，跨越7列，設置上邊距

        # 年份選擇
        year_label = tk.Label(year_month_frame, text="Year:", bg="white")  # 在年份和月份框架中創建一個白色背景的標籤，顯示"Year:"
        year_label.grid(row=0, column=0, padx=5, pady=5, sticky="e")  # 將年份標籤放置到年份和月份框架中，設置在第一行第一列，設置左右邊距，設置上下邊距，設置靠右對齊
        ...
        # 月份選擇
        month_label = tk.Label(year_month_frame, text="Month:", bg="white")  # 在年份和月份框架中創建一個白色背景的標籤，顯示"Month:"
        month_label.grid(row=0, column=2, padx=5, pady=5, sticky="e")  # 將月份標籤放置到年份和月份框架中，設置在第一行第三列，設置左右邊距，設置上下邊距，設置靠右對齊
        ...
        for i in range(6):
            ...
            for j in range(7):
                if mycalendar[i][j] != 0:#列內的有效日期不等於零
                    cell_label = tk.Button(frame, text=mycalendar[i][j], bg="white", relief="ridge", width=6, height=2, borderwidth=1)  # 創建一個白色背景的標籤，顯示該日期，設置邊框樣式
                    ...
                    if mycalendar[i][7]!=7:#列內的有效日期不等於七
                        cell_label = tk.Button(frame, text="", bg="white", relief="ridge", width=6, height=2, borderwidth=1)  # 創建一個白色背景的標籤，不顯示任何內容，設置邊框樣式
                        cell_label.grid(row=i+1, column=j, padx=0, pady=0)  # 將日期標籤放置到日曆框架中的指定位置，設置左右邊距，設置上下邊距
                        row_labels.append(cell_label)  # 將日期標籤添加到該行的標籤列表中
                j+=1
            i+=1
    ...
    def update_calendar(self):
        ...
        calendar_frame.grid(row=1, column=0, columnspan=7)  # 將日曆框架放置到主框架中，設置在第二行，跨越7列
